const pipesAndcitersns = document.querySelector("#sam1");
...
next2.onclick = () => {
    ...
    if (que_count < PIPES_CISTERNS.length - 1) {
        que_count++;
        que_numb++;
...
function showQuestionpipe(index) {
    ...
    let que_tag = '<span>' + PIPES_CISTERNS[index].numb + ". " + PIPES_CISTERNS[index].question + '</span>';
    let option_tag = '<span class="option">' + PIPES_CISTERNS[index].options[0] + '</span>'
        + '<span class="option">' + PIPES_CISTERNS[index].options[1] + '</span>'
        + '<span class="option">' + PIPES_CISTERNS[index].options[2] + '</span>'
        + '<span class="option">' + PIPES_CISTERNS[index].options[3] + '</span>';
...
// change pipes n cisterns questions-------------------------------------------------------------------
function optionSelectedpipe(answer) {
    let userAns = answer.textContent;
    let correctAns = PIPES_CISTERNS[que_count].answer;
